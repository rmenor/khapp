...
export function RequestActions({ request, onActionComplete }: RequestActionsProps) {
	...
	const handleParalyze = async () => {
		console.log('=== handleParalyze CALLED ===');
		console.log('Request to paralyze:', request);
		if (!request?.id) {
			toast({
				variant: 'destructive',
				title: 'Error',
		...
		try {
			...
			console.log('paralyzeRequestAction result:', result);
			...
			if (result.success) {
				...
				toast({
					variant: 'destructive',
					title: 'Error al paralizar',
					description: result.message || 'No se pudo paralizar la solicitud.',
				});
			}
		} catch (error: any) {
			console.error('Unexpected error in handleParalyze:', error);
			toast({
				variant: 'destructive',
				title: 'Error inesperado',
				description: error.message || 'Ocurrió un error inesperado al paralizar la solicitud.',
			});
		} finally {
					...
					<div className="relative bg-background p-6 rounded-lg shadow-lg max-w-md w-full">
						...
						<div className="flex justify-end gap-2">
							...
							<Button
								type="button"
								onClick={() => {
									console.log('Paralyze button clicked - calling handleParalyze');
									handleParalyze();
								}}
